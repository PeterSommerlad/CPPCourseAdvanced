...
using ModuleNodePtr = std::shared_ptr<struct ModuleNode>;
struct cyclic_dependency{};
struct ModuleNode {
  std::string name{};
  std::vector<ModuleNodePtr> required{};
  ...
  struct AllModules {
    ...
    ModuleNodePtr insert(std::string modulename) &;
    void scheduleAll() & {
  ...
  bool isReady(unsigned semester){
    return all_of(begin(required),end(required),[semester](auto const &nodeptr){
      return nodeptr && nodeptr->semester < semester;
    });
  }
