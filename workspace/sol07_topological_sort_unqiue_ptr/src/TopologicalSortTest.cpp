...
void testAddRequiredModule(){
  ...
  ad1.get().addRequired("Math", theModules);
  ASSERT(not ad1.get().isReady(1u));
  ASSERT(math.get().isReady(1u));
  ASSERT(math.get().schedule(1u));
  ASSERT(ad1.get().isReady(2u));
}
...
void testPartitionLevel1(){
  ...
  ASSERT(all_of(begin(theModules.all),it,[](auto const& mp){ return mp && mp->semester==1u;}));
...
void testPartitionLevel2(){
  ...
  ASSERT(all_of(it1,it2,[](auto const& mp){ return mp && mp->semester==2u;}));
  std::vector<std::string> secondSemester{"AD1", "CPI", "DB1"};
  ASSERT(equal(it1,it2,begin(secondSemester),end(secondSemester), [](auto const &l, std::string const &r){ return l->name == r;}));
}
...
void testPartitionLevel3(){
  ...
  ASSERT(all_of(it2,it3,[](auto const& mp){ return mp && mp->semester==3u;}));
  std::vector<std::string> secondSemester{"DB2", "SE1", "UI1"};
  ASSERT(equal(it2,it3,begin(secondSemester),end(secondSemester), [](auto const &l, std::string const &r){ return l->name == r;}));
}
void testPartitionLevel4(){
  ...
  ASSERT(all_of(it3,it4,[](auto const&mp){ return mp && mp->semester==4u;}));
  std::vector<std::string> secondSemester{"SE2",  "UI2"};
  ASSERT(equal(it3,it4,begin(secondSemester),end(secondSemester), [](auto const& l, std::string const &r){ return l->name == r;}));
}
