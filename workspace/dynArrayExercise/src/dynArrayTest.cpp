...
void rangeconstructeddynArray(){
  auto iter { ps_counter::iterator{0} };
  auto e { ps_counter::iterator{11} };
  dynArray<unsigned> const da(iter,e);
  ASSERT_EQUAL(11,da.size());
  ASSERT_EQUAL(1u,da.at(1));
  ASSERT_EQUAL(10u,da.at(-1));
  ASSERT_EQUAL(5u,da[5]);
  ASSERT_EQUAL(6u,da[-5]);
}
...
void testCTADfromRange() {
  std::string s{"0123456789"};

  dynArray const da(begin(s),end(s));
  ASSERT_EQUAL(s.size(),da.size());
  ASSERT_EQUAL('1',da.at(1));
  ASSERT_EQUAL('9',da.at(-1));
  ASSERT_EQUAL('5',da[5]);
  ASSERT_EQUAL('6',da[-4]);
}
...
void runAllTests(int argc, char const *argv[]){
  ...
  s.push_back(CUTE(testCTADfromRange));
